...
dev.off()
png('images/histD.jpeg', height = 700, width = 700, res = 100)
hist(D, main = 'Histogram of Eigenvalues', xlab = 'lambda')
dev.off()
```
...
nest = nested_decomp_mats(outsim$groups)
for (j in 1:3){
  dfr = rasterFromXYZ(cbind.data.frame(outsim$coord, nest$decomp_mats[[j]]%*%outsim$X))              
  image(dfr, main = paste('X level', j), xlab = '', ylab = '', col = viridis(n = 20))
}
dev.off()
...
nest = nested_decomp_mats(outsim$groups)
for (j in 1:3){
  dfr = rasterFromXYZ(cbind.data.frame(outsim$coord, nest$decomp_mats[[j]]%*%outsim$Y))              
  image(dfr, main = paste('X level', j), xlab = '', ylab = '', col = viridis(n = 20))
}
dev.off()
...
nest = nested_decomp_mats(outsim$groups)
for (j in 1:3){
  dfr = rasterFromXYZ(cbind.data.frame(outsim$coord, nest$decomp_mats[[j]]%*%outsim$Z))              
  image(dfr, main = paste('X level', j), xlab = '', ylab = '', col = viridis(n = 20))
}
dev.off()
```
